...
EVE supports flexible authentication through multiple methods. The server attempts authentication in this priority order:
...
class EVEChatClient {
  ...
  setupEventHandlers() {
    this.ws.onopen = () => console.log('Connected to EVE chat service');
    ...
    this.ws.onclose = (event) => {
      if (event.code === 1008) {
        ...
        console.log('Disconnected from EVE chat service');
      }
    };
  }
...
// Usage
const client = new EVEChatClient('your-api-key');
...
class EVEChatClient:
    ...
    async def connect(self):
        ...
        print("Connected to EVE chat service")
...
# Usage
async def main():
    client = EVEChatClient("your-api-key")
...
services:
  eve-chat:
    image: your-eve-image
    ports:
      - "8000:8000"
    environment:
      - CHAT_API_KEY=your-secret-key
      - ANTHROPIC_API_KEY=your-anthropic-key
      - DATABASE_URL=postgresql://user:pass@postgres:5432/evedb
    volumes:
...
*EVE Chat API - Version 1.0 | Last updated: June 2025*